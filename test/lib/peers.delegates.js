...
describe("Peers delegates transactions", function () {
	...
	it("Register delegate and then register username where username name is empty string. Should return not ok", function (done) {
		node.api.post('/accounts/open')
			.set('Accept', 'application/json')
			.send({
				...
				node.api.put('/transactions')
					...
					.end(function (err, res) {
						node.onNewBlock(function (err) {
							...
							var transaction = node.crypti.delegate.createDelegate(account3.password, node.randomDelegateName());
							...
							node.peer.post('/transactions')
								...
								.end(function (err, res) {
									...
									transaction = node.crypti.username.createUsername(account3.password, "");
									node.peer.post('/transactions')
										.set('Accept', 'application/json')
										.send({
											transaction: transaction
