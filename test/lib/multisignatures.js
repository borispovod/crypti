...
var NoXCRAccount = node.randomAccount();
NoXCRAccount.name = "noxcr";
        ...
        node.api.post('/accounts/open')
            ...
            .end(function (err, res) {
                ...
                if (res.body.account != null && i != null) {
                    ...
                    NoXCRAccount.address = res.body.account.address;
                    NoXCRAccount.publicKey = res.body.account.publicKey;
                }
    ...
    node.onNewBlock(function(err) {
        ...
        node.api.put('/transactions')
            ...
            .end(function (err, res) {
                ...
                console.log("Sending " + randomXCR + " XCR to " + account.address);
                node.expect(res.body).to.have.property("success").to.be.true;
                if (res.body.success == true && i != null){
                    Accounts[i].balance = randomXCR / node.normalizer;
                }
            });
    });
};
...
function sendXCRfromMultisigAccount (amount, recipient){
    node.api.put('/transactions')
        ...
        .end(function (err, res) {
            console.log(JSON.stringify(res.body));
            console.log("Sending " + amount + " XCR to " + recipient);
            node.expect(res.body).to.have.property("success").to.be.true;
...
describe('Multisignatures', function() {
    ...
    before(function (done) {
       for (var i = 0; i < (Accounts.length); i++){
           if(Accounts[i] != null){
               sendXCR(Accounts[i], i);
           }
       }
       sendXCR(MultisigAccount, null);
       done();
    });
    ...
    describe('Create Multisignature Account', function() {
        ...
        test += 1;
        it(test + '. Account with 0 XCR. We expect error', function (done) {
            node.api.put('/multisignatures')
                .set('Accept', 'application/json')
                .send({
                    secret: NoXCRAccount.password,
                    lifetime: 1,
                    min: requiredSignatures,
                    keysgroup: Keys
                })
    ...
    describe('Sign Pending Multisignature', function() {
        ...
        test += 1;
        it(test + '. Try to send XCR FROM multisignature account (confirmations still pending). We expect success',function(done) {
            node.onNewBlock(function (err) {
                sendXCRfromMultisigAccount(100000000, node.Faccount.address);
                done();
            });
        });
    });
