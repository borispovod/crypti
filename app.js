...
/**
 * Crypti application instance
 * @param {{}} appConfig Crypti configuration object.
 * @constructor
 */
function Crypti (appConfig) {
	var self = this;

	EventEmitter.call(this);

	// TODO Move logger and domain out of crypti instance. Replace logger with error and log (or so) methods
	// to notify crypti instance of error events.
	...
	d.run(function () {
		...
		async.auto({
			...
			dbLite: function (cb) {
				...
				dbLite.connect(appConfig.blockchain || config.db, cb);
			},
...
inherits(Crypti, EventEmitter);

/**
 * Make crypti app ready state flag isReady to true and emit 'ready' event callbacks.
 */
Crypti.prototype.setReady = function(){
...
/**
 * Add callback for 'ready' event.
 * @param {function} callback
 */
Crypti.prototype.ready = function(callback) {
	if (this.isReady) {
...
/**
 * Emit exit event on crypti application
 */
Crypti.prototype.exit = function() {
	this.emit('exit');
};